...
推荐使用配套扩展 [SnagFlow](https://snagflow.oinchain.com) 把任意网页变成稳定 RSS，再在 RSSFlow 中 **一键导入**。

详见帮助：**订阅源获取 (SnagFlow) → 配置 SnagFlow 导入**。
...
Use the companion extension [SnagFlow](https://snagflow.oinchain.com) to turn almost any page into a stable RSS feed, then import it into RSSFlow in one click.

See **Feed Acquisition (SnagFlow) → Configure SnagFlow Import**.
